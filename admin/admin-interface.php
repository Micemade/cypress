...
<div class="wrap" id="of_container">
  ...
  <form id="of_form" method="post" action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" enctype="multipart/form-data" >
	...
	<a href="http://themeforest.net/user/aligatorstudio" target="_blank" class="as-links">Contact Us</a> | <a href="'.get_template_directory_uri().'/documentation/" target="_blank" class="as-links">Documentation</a>
	...
	<button id ="of_save" type="button" class="button-primary"><?php _e('Save All Changes','cypress');?></button>
	</div><!--.info_bar--> 	
	...
	<button id ="of_save" type="submit" class="button-primary"><?php _e('Save All Changes','cypress');?></button>
	<button id ="of_reset" type="submit" class="button submit-button reset-button" ><?php _e('Options Reset','cypress');?></button>
	</div><!--.save_bar--> 
...
function of_style_only(){
	...
	wp_enqueue_style('jquery-ui-custom-admin', ADMIN_DIR .'css/jquery-ui-custom.css');
}	
...
function of_load_only() {
	...
	wp_enqueue_script('tipsy', ADMIN_DIR .'js/jquery.tipsy.js', array( 'jquery' ));
...
function of_admin_head() { 
	...
	$('#of_backup_button').live('click', function(){
		...
		var answer = confirm("<?php _e('Click OK to backup your current saved options.','cypress');?>")
	...
	$('#of_restore_button').live('click', function(){
		...
		var answer = confirm("<?php _e('Warning: All of your current options will be replaced with the data from your last backup! Proceed?','cypress');?>")
	...
	$('#of_reset').click(function() {
		var answer = confirm("<?php _e('Click OK to reset. All settings will be lost!','cypress');?>")
...
function of_ajax_callback() {
	...
	elseif($save_type == 'restore_options'){
		...
		$of_cypress = get_option('backups');
		
		update_option(OPTIONS, $of_cypress);
	...
	elseif($save_type == 'import_options'){
		...
		$of_cypress = unserialize(base64_decode($_POST['data'])); //100% safe - ignore theme check nag
		
		update_option(OPTIONS,$of_cypress);
		generate_options_css($of_cypress); //generate static css file
		
		die('1'); 
	}
	elseif ($save_type == 'save') {
		
		parse_str(stripslashes($_POST['data']), $of_cypress);
		unset($of_cypress['security']);
		unset($of_cypress['of_save']);
...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			if(!empty($of_cypress) || !empty($data)){
			
				if (array_key_exists('id', $value) && !isset($of_cypress[$value['id']])) {
					$of_cypress[$value['id']] = $value['std'];
					if ($value['type'] == "checkbox" && $value['std'] == 0) {
						$of_cypress[$value['id']] = 0;
					} else {
						$update_data = true;
					}
				}
				if (array_key_exists('id', $value) && !isset($smof_details[$value['id']])) {
					$smof_details[$value['id']] = $of_cypress[$value['id']];
				}
			...
			//Start Heading of each option ///////////////////////
			 if ( $value['type'] != "heading" )
			 {
				...
				if (array_key_exists("fold",$value)) {
					if (isset($of_cypress[$value['fold']]) && $of_cypress[$value['fold']]) {
						$fold="f_".$value['fold']." ";
					} else {
			...
			case 'text':
				...
				if(!isset($value['multi'])) {
					...
					$t_value = '';
					$t_value = stripslashes($of_cypress[$value['id']]);
					...
					$saved_values = $of_cypress[$value['id']];
			...
			case 'select':
				...
				foreach ($value['options'] as $select_ID => $option) {			
					$output .= '<option id="' . $select_ID . '" value="'.$select_ID.'" ' . selected($of_cypress[$value['id']], $select_ID, false) . ' />'.
					$option.'</option>';	 
				 } 
			...
			case 'select_category':
				...
				foreach ($value['options'] as $select_ID => $option) {			
					$output .= '<option id="' . $select_ID . '" value="'.$option[1].'" ' . selected($of_cypress[$value['id']], $option[1], false) . ' />'.
					$option[0].'</option>';	 
				};
			...
			case 'textarea':	
				...
				if(isset($value['options'])){
					...
					$ta_value = stripslashes($of_cypress[$value['id']]);			
					$output .= '<textarea class="of-input" name="'.$value['id'].'" id="'. $value['id'] .'" cols="'. $cols .'" rows="8">'.$ta_value.'</textarea>';		
			break;
			////////////////////////////////////////
			case "radio":
				 
				 if( isset( $of_cypress[$value['id']] ) ) {
					$saved_values = $of_cypress[$value['id']];
				 }
			...
			case 'checkbox':
				...
				$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($of_cypress[$value['id']], 1, false) .' />';
			break;
			////////////////////////////////////////
			case 'multicheck': 			
				
				if( isset($of_cypress[$value['id']]) ) {
					$multi_stored = $of_cypress[$value['id']];
				}		
			...
			////////////////////////////////////////
			case 'color':		
				$output .= '<input class="of-color" name="'.$value['id'].'" id="'. $value['id'] .'" type="text" value="'. $of_cypress[$value['id']] .'" />';
			break;   
			////////////////////////////////////////
			case 'typography':
			
				$typography_stored = $of_cypress[$value['id']];
			...
			case 'select_google_font':
				...
				$google_font_stored = $of_cypress[$value['id']];
				
				//print_r($of_cypress[$value['options']]);
				...
				$output .= '<option value="'.$select_key.'" ' . selected((isset($of_cypress[$value['id']]))? $of_cypress[$value['id']] : "", $option, false) . ' />'.$option.'</option>';  
				} 
				$output .= '</select></div>';
				...
				if(isset($google_font_stored['weight'])) {
					...
					$weights = array('' => '','300'=>'Light','normal'=>'Normal','bold'=>'Bold');
			...
			case 'border':
				...
				$border_stored = $of_cypress[$value['id']];	
			...
			case 'images':
				...
				$select_value = $of_cypress[$value['id']];
			...
			case "nag_ignore":
				...
				if ( !$of_cypress[$value['id']] ) {
					$output .= '<div class="nag">';
					$output .= $message ;

					$output .= '<p><input type="hidden" class="aq-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="0"/>';
					$output .= '<input type="checkbox" class="nag_input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($of_cypress[$value['id']], 1, false) .' />';
		...
		//$output .= '<input class="position" type="hidden" name="'.$value['id'].'['.$group.']['.$key.']" value="'.$list[0].$list[1].'" id="'.$list[0].'">';
			...
			case 'background':
				...
				$background = $of_cypress[$value['id']];
...
*/
			...
			case 'backup':
				...
				$output .= '<p><strong>'. __('Last Backup : ','cypress').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
			...
			case 'transfer':
				...
				$output .= '<textarea id="export_data" rows="8">'.base64_encode(serialize($of_cypress)) /* 100% safe - ignore theme check nag */ .'</textarea>'."\n";
			...
			case 'sliderui':
				...
				$s_val  = stripslashes($of_cypress[$value['id']]);
			...
			case 'switch':
				...
				$output .= '<input type="checkbox" id="'.$value['id'].'" class="'.$fold.'checkbox of-input main_checkbox" name="'.$value['id'].'"  value="1" '. checked($of_cypress[$value['id']], 1, false) .' />';
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order,$int){
		...
		if (!empty($val['title'])) {
			$slider .= '<li><div class="slide_header"><small>'.stripslashes($val['title']).'</small>';
		} else {
			$slider .= '<li><div class="slide_header"><small>Slide '.$order.'</small>';
		}
