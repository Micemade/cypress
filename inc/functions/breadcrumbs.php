...
function dimox_breadcrumbs() {  
	...
	/* === OPTIONS === */  
    $text['home']     = __('Home','cypress'); // text for the 'Home' link  
    $text['category'] = __('Archive by Category "%s"','cypress'); // text for a category page  
    $text['search']   = __('Search Results for "%s" Query','cypress'); // text for a search results page  
    $text['tag']      = __('Posts Tagged "%s"','cypress'); // text for a tag page  
    $text['author']   = __('Articles Posted by %s','cypress'); // text for an author page  
    $text['404']      = __('Error 404','cypress'); // text for the 404 page  
    ...
    if (is_home() || is_front_page()) {  
        ...
        echo '<nav class="breadcrumbs" xmlns:v="http://rdf.data-vocabulary.org/#">' . sprintf($link, $homeLink, $text['home']) . $delimiter;  
		...
		*/
        ...
        if ( get_query_var('paged') ) {  
            ...
            echo __('Page','cypress') . ' ' . get_query_var('paged');  
            if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
        }
