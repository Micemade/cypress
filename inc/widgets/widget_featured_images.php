...
class Widget_Featured_Post_Images extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'featured-image-widget', 'description' => __('A widget displaying featured content images.', 'cypress') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'featured-content-images' );

		/* Create the widget. */
		parent::__construct( 'featured-content-images', __('Featured content images', 'cypress'), $widget_ops, $control_ops );
	}
				...
				<div class="item-content">
					...
					<div class="item-img">
						...
						<div class="back">
							...
							<a href="<?php echo esc_attr($link); ?>" class="button" title="<?php  echo esc_attr(strip_tags(get_the_title()))  ?>"><div class="fs" aria-hidden="true" data-icon="&#xe065;"></div></a>
	...
	function form( $instance ) {
		...
		$defaults = array( 
			'title' => __('Featured content', 'cypress'),
			'num' => 6,
			'width'=>'45%',
			'image_size' => 'thumbnail' ,
			'types'  => 'post'
		...
		*/
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('Title:', 'cypress'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'num' ); ?>"><?php _e('Number of images:', 'cypress'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('types'); ?>"><?php echo __( 'Post type:','cypress' ); ?></label>
			<select name="<?php echo $this->get_field_name('types'); ?>[]" id="<?php echo $this->get_field_id('types'); ?>" class="widefat" style="height: auto;" size="<?php echo $n ?>" >
			<?php 
		...
		<!-- Image sizes : select -->
		<p>
			<label for="<?php echo $this->get_field_id('image_size'); ?>"><?php echo __( 'Image sizes:','cypress' ); ?></label>
			<?php 
			$sizes = array();
			$image_sizes = array();
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'width' ); ?>"><?php _e('Image width in <b>px</b> or <b>%</b> (height is set auto):', 'cypress'); ?></label>
