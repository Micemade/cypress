...
class AS_Slider_Block extends AQ_Block {
	...
	function form($instance) {
			...
			<?php if( $is_slider_tax ) {?>
				...
				if ($sliders_obj) {
					foreach ($sliders_obj as $sl) {
						$sliders_arr[$sl->slug] = $sl->name ;
					}
				}else{
					$sliders_arr[0] = '';
				}
				echo aq_field_multiselect('sliders', $block_id, $sliders_arr, $sliders); 
				?>
			<?php 
			}else{
				echo '<p class="description">There is no <strong>"Slider"</strong> taxonomy registered.<br /> <br /> Please, install and activate "Aligator Custom Post Types plugin</p>';
			}
			?>
		</div>	
		...
		<div class="description half">
			...
			<label for="<?php echo $this->get_field_id('total_items') ?>">Total items</label>
	...
	function block($instance) {
		...
		global $post, $cypress_woo_is_active;
		...
		}elseif( isset($sliders) && $post_type == 'slide' ) {
			$tax_terms =  $sliders;
			$taxonomy = 'slider';
		}else{
			$tax_terms = '';
		...
		<div id="carousel-<?php echo $block_id; ?>" class="content-block slider">
			...
			<div class="owlcarousel-slider">
				...
				<div class="slide-item<?php echo ' '. $style; echo ' '. $anim; ?>">
					...
					<div class="text">
						...
						($cypress_woo_is_active && $post_type=='product') ? woocommerce_template_loop_price() : null;
						
						echo '</a>';
						
						echo ($tagline || get_the_content()) ? '<div class="addendum">' : '';
						
						echo $tagline ? '<h4>'. esc_html($tagline) .'</h4>' : null;
						
						echo get_the_content() ? '<p>' . apply_filters('as_custom_excerpt', 50, false ) . '</p>' : null;
