<div id="style_switcher" class="demo-controls">
	...
	$theme_folder = $themes_folder . '/cypress.' . $my_theme->Version;
	
	$dl =  get_template_directory_uri() .'/admin/images';
	?>
	
	<div id="toggle" title="Demo options"></div>
	
	<div id="buy"><a href="http://themeforest.net/item/cypress-woocommerce-theme-/6853198?ref=aligatorstudio" title="Buy this item on Theme forest"></a></div>
	
	
	<?php $fonts_array = google_fonts(); ?>
	...
	function demo_colorpicker() {
		...
		$('#style_switcher').find('.input-color-picker').each(function(){
			...
			var $_this	= $(this),
				...
				headerfont	= $('#site-menu, #site-menu a, #site-menu #social a, .header-cart, #site-menu-mobile'),
			...
			$_this.wpColorPicker({
				...
				change : function(event, ui) {
					...
					if( $_this.hasClass('head-backcolor') ) {
						...
						$('.header-cart').css( 'background-color', ui.color.toString() );
					...
					if( $_this.hasClass('head-fontcolor') ) {
						...
						$('.header-cart').css( 'color', ui.color.toString() );
	 ...
	 */
		...
		$('#style_switcher #toggle').click(function() {
			if( !switcher ) {
				...
				$(this).parent().animate({'right':-202},{'duration': 200, easing: 'easeInOutQuart'});
				switcher = false;
			}
		});
